...
The [official competition page](https://ebim-benchmark.github.io/competition.html) defines Phase II as real-world validation on Mobile FR3 Duo at Hamburg, Munich and Shanghai. The published general schedule lists September testing dates, but the [current Phase II submission form](https://github.com/EBiM-Benchmark/submissions/blob/main/.github/ISSUE_TEMPLATE/phase2-submission.yml), checked September 12, says deadlines differ by team: **use the deadline in AirSign's advancement email**. Tasks rank independently. Official scoring is run by organizers; the public simulator evaluation scripts are explicitly described as development aids.
...
The repository README also describes a technical-report path weighted at 0.65 of the policy scale. However, on September 12 the issue-template listing contained only `config.yml` and `phase2-submission.yml`. Confirm report-route applicability for Phase II. The current Phase II form asks about **externally provided real object poses**, including operator input and hard-coded targets, rather than the historical simulator-ground-truth declaration. No route description permits replacing measured real observations with fabricated state.
